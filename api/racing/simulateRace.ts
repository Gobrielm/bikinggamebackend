...
class CyclistGroup {
  ...
  simulateTick(): void {
	this.simulateCyclists();
	this.simulateMovement();
  }

	simulateCyclists(): void {
		{
			let draft = this.getDraft(0);
			let speedTotal = 0;
			let count = 0;
			for (let cyclist of this.cyclists[0]) {
				if (!cyclist) continue;

				cyclist.simulateTick(draft, this.route);
				speedTotal += cyclist.velocity;
				count++;
			}
			this.velocity = speedTotal / count;
		}
		

		for (let row = 1; row < this.cyclists.length; row++) {
  	...
  	simulateMovement(): void {
		let cyclistsMovingBack: number[][] = [];

		for (let col = 0; col < this.cyclists[0].length; col++) {
			let cyclist = this.cyclists[0][col];
			if (!cyclist) {
				cyclistsMovingBack.push([0, col]);
				continue;
			}

			// Moving Up
			if (cyclist.velocity > this.velocity * 1.1) {
				// TODO: create new group
			// Moving back
			} else if (cyclist.velocity * 1.1 < this.velocity) {
				cyclistsMovingBack.push([0, col]);
			}
		}
		...
		for (let row = 1; row < this.cyclists.length; row++) {
			let cyclistRow = this.cyclists[row];
			for (let col = 0; col < cyclistRow.length; col++) {
				let cyclist = cyclistRow[col];
				if (!cyclist) continue;

				// Moving Up
				if (cyclist.velocity > this.velocity * 1.1) {
					if (cyclistsMovingBack.length > 0) {
						this.swapCyclists([row, col], cyclistsMovingBack[cyclistsMovingBack.length - 1]);
						cyclistsMovingBack.pop();
					} else {
						// TODO
					}
				// Moving back
					...
					if (cyclistsMovingBack.length > 0) {
						// TODO: Cyclist moving back takes this place
					} else {
						cyclistsMovingBack.push([row, col]);
					}
				}
			}
		}

    // Deal with leaving Group
  ...
  // What percentage of wind is blocked
	...
	swapCyclists(index1: number[], index2: number[]) {
		let temp = this.cyclists[index1[0]][index1[1]];
		this.cyclists[index1[0]][index1[1]] = this.cyclists[index2[0]][index2[1]];
		this.cyclists[index2[0]][index2[1]] = temp;
	}

	moveCyclistUp(row: number, col: number): void {
		let cyclist = this.cyclists[row][col];
	...
	addCyclistToRow(cyclist: Cyclist, row: number, col: number): void {
		...
		for (let j = 0; j < this.cyclists[row].length; j++) {
			if (this.cyclists[row][j] != null) continue;
	...
	addCyclistToBack(cyclist: Cyclist): void {
		for (let i = this.cyclists.length; i >= 0; i--) {
			for (let j = 0; j < this.cyclists[0].length; j++) {
				if (this.cyclists[i][j] != null) continue;
